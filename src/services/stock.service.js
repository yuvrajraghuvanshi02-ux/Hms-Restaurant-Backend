...
const addStockWithClient = async (client, { raw_material_id, quantity, unit_id }) => {
...
const addStock = async (tenantPool, { raw_material_id, quantity, unit_id }) => {
  ...
  return await withTenantTx(tenantPool, async (client) => {
    return await addStockWithClient(client, { raw_material_id: rawMaterialId, quantity: qty, unit_id: unitId });
  });
};

const deductStockByVariantWithClient = async (client, variantId, { multiplier = 1 } = {}) => {
  const vId = ensureUuid(variantId, "menu_item_variant_id");
  const mult = toPositiveNumber(multiplier, "multiplier");
  ...
  const requirements = rows.rows.map((r) => {
    const perUnit = toPositiveNumber(r.recipe_quantity, "quantity");
    const reqQty = perUnit * mult;
...
const deductStockByVariant = async (tenantPool, variantId) => {
  return await withTenantTx(tenantPool, async (client) => {
    return await deductStockByVariantWithClient(client, variantId, { multiplier: 1 });
  });
};
...
module.exports = {
  ...
  deductStockByVariantWithClient,
};
